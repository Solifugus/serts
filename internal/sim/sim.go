...
func (s *State) TotalCoin() float64 {
	...
	for i := range s.Structs {
		if s.Structs[i].Alive {
			st := &s.Structs[i]
			// Gold is the building's own money and Works is the hall's public-works
			// fund. Both are real coin in a real place, and leaving either out makes
			// money appear to vanish when it has merely been moved. Works was omitted
			// until a bank was built and then reverted, and the conservation test could
			// not catch it: that test only looks for money being CREATED, so a pool
			// quietly draining out of the count reads as success.
			held += float64(st.Gold) + float64(st.Works)
		}
	}
